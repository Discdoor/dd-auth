...
describe('User manager', () => {
    ...
    // Test case: resolve user tag
    test('resolve user by tag', async() => {
      const TAG = `${testUser.username}#${testUser.discrim}`;
      const user = await umgr.getUserByTag(TAG);
      console.log(testUser);
      expect(user).toBeDefined();
      expect(user).not.toBeNull();
      expect(user.username).toBe(testUser.username);
      expect(user.discrim).toBe(testUser.discrim);
    });
