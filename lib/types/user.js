...
const { assignProps, createShallowView } = require('libdd-node').reflect;
...
class User {
    ...
    async changePassword(password) {
        ...
        ensureAcknowledgement(await this._col.updateOne({ id: this.id }, { $set: { passwordHash: hash }}));
        this.passwordHash = hash; // Reflect changes in local object
    }

    /**
     * Changes the email for this user account.
     * @param {String} email The new email to use.
     */
    async changeEmail(email) {
        ensureAcknowledgement(await this._col.updateOne({ id: this.id }, { $set: { email, verifStatus: "AWAIT_VERIF" }}));
        this.email = email; // Reflect changes in local object
    }

    /**
     * Changes the email for this user account.
     * @param {String} username The new email to use.
     */
    async changeUsername(username) {
        ensureAcknowledgement(await this._col.updateOne({ id: this.id }, { $set: { username }}));
        this.username = username; // Reflect changes in local object
    }
    ...
    async setAvatarUrl(url) {
        ...
        ensureAcknowledgement(await this._col.updateOne({ id: this.id }, { $set: { avatarUrl: url }}));
        this.avatarUrl = url;
    }
    ...
    async updateLoginDate() {
        let updatedLDate = new Date();
        ensureAcknowledgement(await this._col.updateOne({ id: this.id }, { $set: { lastLoginDate: updatedLDate }}));
        this.lastLoginDate = updatedLDate;
    }
    ...
    /**
     * Creates a user view for caching.
     */
    createCacheView() {
        return createShallowView(this, ["passwordHash", "phone", "verifStatus", "lastLoginDate", "dateOfBirth", "email"]);
    }
}
